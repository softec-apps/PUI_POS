'use client'

import { I_Brand } from '@/common/types/modules/brand'
import { ConfirmationModal } from '@/components/layout/templates/ConfirmationModal'

interface RestoreModalProps {
	isOpen: boolean
	currentRecord: I_Brand | null
	isAction: boolean
	onClose: () => void
	onConfirm: () => Promise<void>
}

export function RestoreModal({ isOpen, currentRecord, isAction, onClose, onConfirm }: RestoreModalProps) {
	return (
		<ConfirmationModal
			isOpen={isOpen}
			variant='success'
			title='Restaurar marca'
			description='Esta acción se puede deshacer'
			message={
				<>
					¿Deseas restaurar la marca <span className='text-foreground font-semibold'>{currentRecord?.name}</span>?
				</>
			}
			alertMessage={
				<>
					<p className='text-emerald-600 dark:text-emerald-400'>La marca volvera ha estar disponible en el sistema.</p>
				</>
			}
			isProcessing={isAction}
			onClose={onClose}
			onConfirm={onConfirm}
			confirmText='Restaurar'
			cancelText='Cancelar'
		/>
	)
}
