...
const fitText = (doc: jsPDF, text: string, x: number, y: number, maxWidth: number, maxLines = 2, lineHeight = 4) => {
...
export const generateSaleReceiptPDF = (saleData: SaleToTicket = {}) => {
	...
	// === ENCABEZADO ===
	if (saleData.establishment?.companyName) center(saleData.establishment.companyName, 9, true, 5)
	if (saleData.establishment?.tradeName) center(saleData.establishment.tradeName, 8, false, 4)
	if (saleData.establishment?.ruc) center(`RUC: ${saleData.establishment.ruc}`, 7, false, 3)
	...
	doc.text(`Cliente: ${c.firstName || 'Consumidor Final'} ${c.lastName || ''}`, leftX, y)
	y += 4
	...
	saleData.items?.forEach(item => {
		...
		doc.text(formatPrice(item.unitPrice * item.quantity), rightX, y, { align: 'right' })
		y += used + 1
	})
	...
	// Nombre del archivo
	const fileName = clave ? `ticket_${clave}.pdf` : `ticket_${saleData.id}.pdf`
	doc.save(fileName)
}
