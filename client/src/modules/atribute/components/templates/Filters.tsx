...
interface Props {
	...
	currentSort?: string
	currentRequired?: 'true' | 'false' | ''
	onSearchChange: (e: React.ChangeEvent<HTMLInputElement>) => void
	onSort: (sortKey: string) => void
	onRequiredChange: (required: 'true' | 'false' | '') => void
	onRefresh: () => void
	onResetAll: () => void
	viewType: ViewType
...
const requiredOptions: { key: 'true' | 'false' | ''; label: string; color: string }[] = [
	{ key: '', label: 'Todos', color: 'bg-accent-foreground/40' },
	{ key: 'true', label: 'Requerido', color: 'bg-green-500' },
	{ key: 'false', label: 'Opcional', color: 'bg-red-500' },
]
...
export function AttributeFilters({
	...
	const activeFiltersCount = [searchValue.length > 0, currentRequired !== '', currentSort !== ''].filter(
		Boolean
	).length
	...
	const getCurrentRequiredLabel = () => {
		if (!currentRequired) return 'Filtro'
		const option = requiredOptions.find(opt => opt.key === currentRequired)
				...
				<motion.div
					...
					<div className='flex items-center gap-4'>
						...
						<motion.div
							...
							<DropdownMenu>
								<DropdownMenuTrigger asChild>
									<ActionButton icon={<Icons.filter />} text={getCurrentRequiredLabel()} variant='ghost' />
								</DropdownMenuTrigger>
								...
								<DropdownMenuContent
									...
									{requiredOptions.map((option, index) => (
										<DropdownMenuItem
											key={option.key}
											onClick={() => onRequiredChange(option.key)}
											className='hover:bg-accent/80 text-accent-foreground/75 cursor-pointer rounded-lg transition-all duration-200'>
											<motion.div
												className='flex w-full items-center justify-between'
												initial={{ opacity: 0, x: -10 }}
												...
												<div className='flex items-center gap-2'>
													...
													<span
														className={
															currentRequired === option.key ? 'text-primary font-medium' : ''
														}>
														{option.label}
													</span>
												</div>
												{currentRequired === option.key && (
													<motion.div
														className='bg-primary h-2 w-2 rounded-full'
														initial={{ scale: 0 }}
														animate={{ scale: 1 }}
									...
									<Badge
										...
										<div
											className={`h-2 w-2 rounded-full ${
												requiredOptions.find(opt => opt.key === currentRequired)?.color
											}`}
										/>
										<span>{getCurrentRequiredLabel()}</span>
										<button
											onClick={() => onRequiredChange('')}
											className='hover:bg-muted-foreground text-muted-foreground hover:text-muted cursor-pointer rounded-full p-0.5 transition-all duration-500'>
											<Icons.x className='h-3 w-3' />
