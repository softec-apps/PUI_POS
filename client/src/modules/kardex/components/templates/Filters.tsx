...
interface Props {
	...
	currentSort?: string
	currentMovementType?: I_Kardex['movementType'] | ''
	onSearchChange: (e: React.ChangeEvent<HTMLInputElement>) => void
	onSort: (sortKey: string) => void
	onMovementTypeChange: (movementType: I_Kardex['movementType'] | '') => void
	onRefresh: () => void
	onResetAll: () => void
	viewType: ViewType
...
const movementTypeOptions: { key: I_Kardex['movementType'] | ''; label: string; color: string }[] = [
	{ key: '', label: 'Todos', color: 'bg-accent-foreground/40' },
	{ key: 'purchase', label: 'Compra', color: 'bg-green-500' },
	{ key: 'return_in', label: 'Devolución (Entrada)', color: 'bg-cyan-500' },
	{ key: 'transfer_in', label: 'Transferencia (Entrada)', color: 'bg-teal-500' },
	{ key: 'sale', label: 'Venta', color: 'bg-purple-500' },
	{ key: 'return_out', label: 'Devolución (Salida)', color: 'bg-indigo-500' },
	{ key: 'transfer_out', label: 'Transferencia (Salida)', color: 'bg-orange-500' },
	{ key: 'adjustment_in', label: 'Ajuste (Entrada)', color: 'bg-blue-500' },
	{ key: 'adjustment_out', label: 'Ajuste (Salida)', color: 'bg-yellow-500' },
	{ key: 'damaged', label: 'Dañado', color: 'bg-red-500' },
	{ key: 'expired', label: 'Expirado', color: 'bg-gray-500' },
]
...
export function KardexFilters({
	...
	const getCurrentMovementTypeLabel = () => {
		...
		const option = movementTypeOptions.find(opt => opt.key === currentMovementType)
						...
						<motion.div
							...
							<DropdownMenu>
								...
								<DropdownMenuContent
									...
									<DropdownMenuSeparator />
									{movementTypeOptions.map((option, index) => (
										<DropdownMenuItem
											key={option.key}
											onClick={() => onMovementTypeChange(option.key)}
											className='hover:bg-accent/80 text-accent-foreground/75 cursor-pointer rounded-lg transition-all duration-200'>
											<motion.div
												className='flex w-full items-center justify-between'
												initial={{ opacity: 0, x: -10 }}
												...
												{currentMovementType === option.key && (
													<motion.div
														className='bg-primary h-2 w-2 rounded-full'
														initial={{ scale: 0 }}
														animate={{ scale: 1 }}
									...
									<Badge
										...
										<div
											className={`h-2 w-2 rounded-full ${
												movementTypeOptions.find(opt => opt.key === currentMovementType)?.color
											}`}
										/>
										<span>{getCurrentMovementTypeLabel()}</span>
										<button
											onClick={() => onMovementTypeChange('')}
											className='hover:bg-muted-foreground text-muted-foreground hover:text-muted cursor-pointer rounded-full p-0.5 transition-all duration-500'>
											<Icons.x className='h-3 w-3' />
